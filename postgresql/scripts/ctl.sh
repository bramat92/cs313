...
HOSTNAME=`hostname`
POSTGRESQL_PIDFILE=/Applications/mappstack-5.6.32-1/postgresql/data/postmaster.pid

POSTGRESQL_START="/Applications/mappstack-5.6.32-1/postgresql/bin/pg_ctl start -w -D /Applications/mappstack-5.6.32-1/postgresql/data"
POSTGRESQL_STOP="/Applications/mappstack-5.6.32-1/postgresql/bin/pg_ctl stop -D /Applications/mappstack-5.6.32-1/postgresql/data"
...
start_postgresql() {
    ...
    if [ $ERROR -eq 0 ]; then
	echo "$0 $ARG: postgresql  started at port 5000"
	sleep 2
    else
