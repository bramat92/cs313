...
CREATE OR REPLACE FUNCTION standardize_address(
        ...
        macro text )
    RETURNS stdaddr
    AS  '$libdir/address_standardizer-2.4', 'standardize_address'
...
CREATE OR REPLACE FUNCTION standardize_address(
        ...
        address text )
    RETURNS stdaddr
    AS  '$libdir/address_standardizer-2.4', 'standardize_address1'
    LANGUAGE 'c' IMMUTABLE STRICT COST 200;
...
CREATE OR REPLACE FUNCTION parse_address(IN text,
        ...
        OUT country text)
    RETURNS record
    AS  '$libdir/address_standardizer-2.4', 'parse_address'
