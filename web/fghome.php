	...
	<body>
		...
		<div class="container">
			...
			<?php	
				...
				if (isset($_GET['sbtn'])){
					$text = $_GET['search'];
					$ptext = $db->prepare('SELECT post, CONCAT(firstname, " ", lastname) AS name, to_char(posts.created_at, \'YYYY/MM/DD\') AS date FROM posts JOIN users ON users.id = posts.user_id WHERE name LIKE :keyword ORDER BY date DESC');
					$text = "%".$text."%";
