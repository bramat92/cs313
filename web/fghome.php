	...
	<body>
		...
		<div class="container">
			...
			<?php
				...
				foreach ($db->query('SELECT post, username, created_at FROM users JOIN posts ON users.id = posts.user_id WHERE users.id = (SELECT id FROM users WHERE username = \'bernie\')') as $row)
				{
					echo '<p>';
